...
describe("applyDiscount()", () => {
  it("does not discount the cost if the user is not subscribed", () => {
    expect(applyDiscount(100, false)).toBe(100);
  });

  it("discounts the cost by 25% if the user is subscribed", () => {
    expect(applyDiscount(100, true)).toBe(75);
  });
});

describe("applyCoupon()", () => {
  it("does not discount the cost if the user does not have a coupon", () => {
    expect(applyCoupon(100, false)).toBe(100);
  });

  it("discounts the cost by $10 if the user has a coupon", () => {
